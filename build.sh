...
BUILD_WEB() {
  git clone https://github.com/TheJan-Jan/alist-web.git
  cd alist-web
  yarn
  yarn build
...
CDN_WEB() {
  curl -L https://github.com/alist-org/TheJan-Jan/releases/latest/download/dist.tar.gz -o dist.tar.gz
...
BUILD_DOCKER() {
  ...
  webTag=$(wget -qO- -t1 -T2 "https://api.github.com/repos/TheJan-Jan/alist-web/releases/latest" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')
  ldflags="\
-w -s \
...
BUILD() {
  ...
  gitCommit=$(git log --pretty=format:"%h" -1)
  gitTag=$(git describe --long --tags --dirty --always)
  webTag=$(wget -qO- -t1 -T2 "https://api.github.com/repos/alist-org/TheJan-Jan/releases/latest" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')
